...
| **Windows server operator (64-bit, preview)** | `dinero-core-2.2.5-rc7.1-windows-win64-preview.zip` | Daemon + CLI only, no GUI — `dinerod.exe`, `dinero-cli.exe`, runtime DLLs, plus `install-service.ps1` / `uninstall-service.ps1` for Windows service registration. Canonical Windows-aware datadir. Extracts to `Dinero-Core/`. ~16 MB. rc7.2 makes no daemon-side changes; operators stay on the [`v2.2.5-rc7.1`](https://github.com/DineroLabs/dinero-releases/releases/tag/v2.2.5-rc7.1) Core zip. |
